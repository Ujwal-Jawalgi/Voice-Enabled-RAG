...
Additionally, dense embedding retrieval can surface passages with high structural/topical similarity but the wrong specific entity. For example, the query "capital of France" retrieved a passage about the "capital of the United States" (Boston) at a high similarity score (0.7987), because both share the semantic pattern "capital of [country]" without the embedding distinguishing which country. The output guardrail correctly caught this: the LLM recognized the retrieved passage didn't actually answer the question asked and refused rather than conflating the pattern match with a real answer. This demonstrates the guardrail layer's value beyond simple off-topic detection — it also catches near-miss entity confusion that a pure similarity threshold cannot.
