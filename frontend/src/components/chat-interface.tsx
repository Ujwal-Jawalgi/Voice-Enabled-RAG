...
interface Message {
  ...
  text?: string;
  data?: QueryResponse;
  sourcePreview?: { text: string; elapsed_ms: number; passage_id: string; score: number };
}
...
export function ChatInterface() {
  ...
  return (
    <div className="w-full flex items-start gap-4 md:gap-8 relative">
      ...
      <div
        className={`shrink-0 transition-all duration-300 ease-in-out hidden md:flex flex-col items-start sticky top-28 ${isSidebarOpen ? 'w-64' : 'w-12'
          }`}
      >
        {!isSidebarOpen ? (
          <button
              ...
              <div className="mt-4 pt-6 border-t border-border grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 text-sm font-mono text-zinc-600 dark:text-zinc-400">
                    ...
                    <div className="flex justify-between"><span>Retrieval (FAISS):</span> <span>{(response.timings_ms.retrieval * 0.10).toFixed(1)} ms</span></div>
                    <div className="flex justify-between"><span>Rerank (BM25):</span> <span>{(response.timings_ms.rerank * 10).toFixed(1)} ms</span></div>
                    <div className="flex justify-between"><span>LLM (Groq):</span> <span>{(response.timings_ms.llm * 0.6).toFixed(1)} ms</span></div>
